...
@mcp.tool()
@log_tool_call
def lldb_initialize_debugger(filename: str, arch: str = "x86_64") -> str:
    """
    ...
    Example:
        lldb_initialize_debugger("/path/to/my/program", "x86_64")
    """
    ...
    return (
        f"Successfully initialized LLDB debugger for target: {filename} (arch: {arch})"
    )
...
def run_lldb_command_func(command: str) -> dict:
    ...
    if not debugger_state["interpreter"] or not debugger_state["target"]:
        return {
            "error": "LLDB not initialized. Call lldb_initialize_debugger() first.",
            "lldb_output": "",
            "program_stdout": "",
        }
...
@mcp.tool()
@log_tool_call
def lldb_run_command(command: str, output_filename: str = "") -> dict:
    """
    ...
    Example:
        lldb_run_command("breakpoint set --name main", "/workspace/project1/lib/lldb_output/output.txt")
        lldb_run_command("run", "/workspace/project1/lib/lldb_output/output.txt")
        lldb_run_command("print myVariable")
    """
...
@mcp.tool()
@log_tool_call
def lldb_set_breakpoint(location: str, condition: str = "") -> dict:
    """
    ...
    Example:
        lldb_set_breakpoint("main")
        lldb_set_breakpoint("utils.cpp:150")
        lldb_set_breakpoint("processData", "count > 100")
    """
    global debugger_state
    ...
    if not debugger_state["interpreter"]:
        return {
            "success": False,
            "message": "LLDB not initialized. Call lldb_initialize_debugger() first.",
            "lldb_output": "",
        }
...
@mcp.tool()
@log_tool_call
def lldb_list_breakpoints() -> dict:
    """
